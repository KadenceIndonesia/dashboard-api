...
exports.getVisitAchievementByDate = async function (req, res) {
  try {
    ...
    for (let i = 0; i < result.date.length; i++) {
      var formatDate = moment(result.date[i]).format('YYYY-MM-DD');
      var _countAchievementTotalDate = await countAchievementTotalDate(
        pid,
        formatDate,
        wave,
        region,
      ...
      result.value[i] = _countAchievementTotalDate;
    }
...
exports.getStatusPangkalan = async function (req, res) {
  try {
    ...
    for (let i = 0; i < result.length; i++) {
      var _countAchievementStatusPangkalan =
        await countAchievementStatusPangkalan(
          pid,
          result[i].code,
          wave,
          region,
      ...
      result[i].count = _countAchievementStatusPangkalan;
      total = total + _countAchievementStatusPangkalan;
    }
...
exports.getStatusOnBoardingPangkalan = async function (req, res) {
  try {
    ...
    result[0].value = await countByQuestionAndAnswer(
      pid,
      'A12',
      '1',
      wave,
    ...
    result[1].value = await countByQuestionAndAnswer(
      pid,
      'A113',
      '2',
      wave,
    ...
    result[2].value = await countByQuestionAndAnswer(
      pid,
      'A12',
      '2',
      wave,
    ...
    result[3].value = await countByAnswerOperationsIn(
      pid,
      'A12',
      ['3', '4'],
      wave,
...
exports.getHelpBoardingPangkalan = async function (req, res) {
  try {
    ...
    result[1].value = await countByAnswerOperationsIn(
      pid,
      'A31',
      ['2', '-1'],
      wave,
...
exports.getSortBoarding = async function (req, res) {
  try {
    ...
    // console.log(await getGroupCity(pid, wave, region, province, city));
    ...
    var response = await getGroupCity(pid, wave, region, province, city);
...
exports.getVisitByProvince = async function (req, res) {
  try {
    ...
    for (let i = 0; i < result.length; i++) {
      var _countVisitProvince = await countVisitProvince(pid, result[i].label);
      result[i].count = _countVisitProvince;
    }
